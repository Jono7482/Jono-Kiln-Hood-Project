...
def locate_points_canvas(canvasx, canvasy, scale, points, face, offset, lengthdif=None):
    # Scale points
    if face == "free":
        offset = 0
    elif face == "Flat":
        ...
        offset = (offset * scale) / 3

    if lengthdif != None:
        lengthdifx = (lengthdif[0] / 2) * scale
        lengthdify = (lengthdif[1] / 2) * scale
    ...
    if (face is "front") or (face is "Flat"):
        return points
    elif face is "side":
        ...
        for x in range(len(points)):
            points[x][0] += (canvasx / 1.4) + lengthdifx
            points[x][1] -= (canvasy / 1.4) - lengthdify
        return points
    elif face is "free":
        for x in range(len(points)):
